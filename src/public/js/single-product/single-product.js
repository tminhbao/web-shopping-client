...
function addToCartClicked(event) {
  ...
  if (userElement == null) {
    alert("Chưa đăng nhập, xin mời đăng nhập");
  } else {
    const name = document.getElementsByClassName("product_name")[0].innerText;
    const imageSrc = document.querySelector(".imageSrc").src;
    const price = parseInt(
      document.querySelector(".new-price").innerText.replace("đ", "")
    );
    const quantity = document.querySelector("#quantity-product").value;
    return { name, imageSrc, price, quantity };
